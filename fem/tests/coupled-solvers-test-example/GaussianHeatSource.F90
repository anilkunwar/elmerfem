FUNCTION TravellingHeatSource( Model, n, t ) RESULT(f)
  ...
  REAL(KIND=dp) :: Alpha, Coeff, Speed, Dist, Dist0, tau, &
      Time, x, y, z, s, sper, r
  ...
  IF( NewTimestep ) THEN
    ...
    tau = GetCReal(Params,'Pulse width', Found)
  ...
  f = Coeff *EXP(-time**2/tau**2)* EXP( -2*r**2 / Alpha**2 )
...
FUNCTION FixedHeatSource( Model, n, t ) RESULT(f)
  ...
  INTEGER :: timestep, prevtimestep = -1
  REAL(KIND=dp) :: Alpha, Coeff, Dist0, tau, &
      Time, x, y, z, r
  TYPE(Mesh_t), POINTER :: Mesh
  TYPE(ValueList_t), POINTER :: Params
  LOGICAL :: Found, NewTimestep
